...
const ContinewCard = ({ data }: { data: Doc<"projects"> }) => {
  return (
    <div className="flex flex-col gap-2">
      ...
      <Button
        variant="outline"
        asChild
        className="h-auto items-start justify-start p-4 bg-background border rounded-cl flex flex-col gap-2"
      >
...
const ProjectItem = ({ data }: { data: Doc<"projects"> }) => {
  return (
    ...
    >
      <div className="flex items-center gap-2">
        {getProjectIcon(data)}
        <span className="text-xm text-muted-foreground group-hover:text-foreground/60 transition-colors">
          {data.name}
        </span>
      </div>
...
export const ProjectsList = ({ onViewAll }: ProjectsListProps) => {
  ...
  return (
    <div className="flex flex-col gap-4">
      {mostRecent ? <ContinewCard data={mostRecent} /> : null}
      {rest.length > 0 && (
        <div className="flex flex-col gap-2">
